...
**The only tests are Playwright e2e for `apps/web`** (`apps/web/e2e/`, `pnpm test:e2e`). There are no unit tests and none for `apps/api` or the packages. `lint` is `tsc --noEmit` in every workspace, and CI (`.github/workflows/ci.yml`) runs lint + `pnpm --filter web build`, an `e2e` job in parallel, then on pushes to `master` a `deploy-api` job ships the API to Fly (only when `apps/api`, `packages/` or the lockfile changed; uses the `FLY_API_TOKEN` repo secret; not gated on e2e). Don't add `version:` to `pnpm/action-setup`; it reads `packageManager` from `package.json` and fails if both are set. E2E passing says nothing about the API or its queries; for server changes, typecheck and exercise the endpoint.

E2E runs a production build of the web (`next build` into `.next-e2e`, no standalone output) against `apps/web/e2e/mock-api/server.mjs`, a fixture server that stands in for the API and records every write (`GET /__e2e/requests`) so specs can assert on what the browser sent. Playwright starts both; the mock must be up before the build, because static pages are prerendered with whatever the API returns. The config forces Clerk off and sets `DATABASE_URL` to a malformed value so the web's exchange-rate lookup falls back to `USD_TO_INR_RATE=84`; values in `.env.local` can't override this, so e2e never reaches Neon. New pages or API fields need matching fixtures in `e2e/mock-api/fixtures.mjs`. `E2E_SKIP_BUILD=1` reuses the last e2e build. First run: `pnpm --filter web exec playwright install chromium`.
...
`packages/job-alerts/src/email.ts` sends through **AgentMail** (`agentmail` SDK), not Resend. It reads `AGENTMAIL_API_KEY` and `AGENTMAIL_INBOX_ID` (`alerts@intelliforge.tech`, shared with other IntelliForge apps) and never throws: a missing key or API error comes back as `{ ok: false, error }`. `handleDigest` still returns 200 when every send fails, so check `emailsSent` and `errors` in its body. `GET /api/cron/digest?to=<email>` restricts a run to one address for testing. Every email is sent as branded HTML plus a plain-text part: the layout lives in `packages/job-alerts/src/templates.ts` (tables and inline styles only, colours copied from `apps/web/app/globals.css`), and ingested titles go through `cleanText()` because they arrive with HTML entities like `&amp;`. The unsubscribe link sits in the footer and in the `List-Unsubscribe` headers. Digests carry HMAC-signed unsubscribe links (`UNSUBSCRIBE_SECRET`, falling back to `CRON_SECRET`); `GET /api/unsubscribe` must stay side-effect free because mail scanners fetch it, and only POST deletes the subscriber. Links use `NEXT_PUBLIC_APP_URL`, falling back to `https://remoteforge.intelliforge.tech`; `remoteforge.in` does not resolve. Setup and testing are in `DEPLOY.md`.
